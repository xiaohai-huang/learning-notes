...
<details>
  ...
  In the <Link to="https://github.com/xiaohai-huang/cab420-workspace/blob/master/work/week1/CAB420_Regression_Example_1_Linear_Regression.ipynb">
    "number of cyclists"
  </Link> case study, one of the linear regression model contains <code>temp</code>, 
  and <code>atemp</code> as parameters, they are correlated, and thus the
  relationship between that variable and the response is (to some extent) captured
  twice in the model. That causes the `p-value` to be less important.
</details>
