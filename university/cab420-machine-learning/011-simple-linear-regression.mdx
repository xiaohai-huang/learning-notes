...
<details>
  ...
  <div>
    ...
    case study, one of the linear regression model contains <code>temp</code> , and{" "}
    <code>atemp</code> as parameters, they are correlated, and thus the relationship
    between that variable and the response is (to some extent) captured twice in
    the model. That causes the `p-value` to be less important.
  </div>
</details>
